...
- 移除本地 `Array.isArray` FFI，改用 dom-ffi 已验证的
  `JsObscure::is_array`。
- 下游发布验证不再硬编码旧版 dom-ffi；它从 `moon.mod` 解析精确版本，并验证
...
- Removes the stale dom-ffi hardcode from downstream release verification. The
  verifier derives the exact version from `moon.mod` and checks that the
  dependency tree contains both target React and dom-ffi versions.
